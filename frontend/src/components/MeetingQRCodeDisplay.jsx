...
export default function MeetingQRCodeDisplay({ meetingId }) {
  ...
  const generateQRCode = async () => {
    try {
      setGenerating(true);
      const response = await api.post(`/meetings/${meetingId}/generate_qr/`);
  ...
  const isQRValid = () => {
    if (!meeting || !meeting.qr_expires_at) return false;
    return new Date(meeting.qr_expires_at) > new Date();
  };

  if (loading) {
    return (
      <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl p-6 border border-gray-700 shadow-xl text-center">
  ...
  return (
    <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl p-6 border border-gray-700 shadow-xl">
      ...
      {meeting && meeting.qr_code && isQRValid() ? (
        <div className="space-y-4">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="text-white">
                {new Date(meeting.qr_expires_at).toLocaleString('tr-TR', {
                  day: '2-digit',
                  month: 'long',
                  hour: '2-digit',
                  minute: '2-digit'
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={generating}
              className="flex-1 bg-red-600 hover:bg-red-700 text-white py-2 px-4 rounded-lg transition-all disabled:opacity-50"
            >
              🔄 Yenile
            </button>
          </div>
        ...
        <div className="text-center space-y-4">
          ...
          {meeting && meeting.qr_expires_at && !isQRValid() ? (
            <>
              <p className="text-gray-400">QR kod süresi dolmuş</p>
              <p className="text-sm text-gray-500">
                Son geçerlilik: {new Date(meeting.qr_expires_at).toLocaleString('tr-TR')}
              </p>
            </>
          ) : (
